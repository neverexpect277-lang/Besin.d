...
function KameraOCR({ onMetin, onIptal }) {
 ...
 alert("OCR motoru yükleniyor, birkaç saniye sonra tekrar dene.");
 return;
 }
 setDurum("isleniyor");
 ...
 if (stream) stream.getTracks().forEach(t => t.stop());
 try {
 const result = await window.Tesseract.recognize(c, "tur+eng", {
 logger: m => { if (m.status === "recognizing text") setIlerleme(Math.round(m.progress * 100)); }
 });
 onMetin(result.data.text);
 } catch (e) {
 alert("Metin okunamadı, manuel girişi dene.");
 setDurum("bekle");
 }
 }
 ...
 <div style={{ width: "85%", height: "45%", border: `2px solid ${C.altin}`, borderRadius: 8, boxShadow: "0 0 0 2000px rgba(0,0,0,0.4)" }}>
 <div style={{ position: "absolute", top: 8, left: "50%", transform: "translateX(-50%)", background: C.altin, borderRadius: 6, padding: "3px 10px" }}>
 <span style={{ color: "#1A1200", fontSize: 11, fontWeight: 700 }}>İçindekiler bölümünü çerçevele</span>
 </div>
 </div>
 </div>
 </div>
...
/* ══════════════════════════════════════════════
 STİLLER
 ══════════════════════════════════════════════ */
const css = `*{box-sizing:border-box;margin:0;padding:0} body{background:#07070F;font-family:Georgia,serif} textarea:focus,input:focus{outline:2px solid #C9A84C50} ::-webkit-scrollbar{width:3px} ::-webkit-scrollbar-thumb{background:#22223A;border-radius:2px}`;
...
const S = {
 ...
 kB: { color: "#C9A84C", fontSize: 10, letterSpacing: 2, fontWeight: 700, marginBottom: 8, marginTop: 6, display: "block" },
 ...
 notUyari: { color: "#45435A", fontSize: 10, fontStyle: "italic", marginBottom: 12, lineHeight: 1.5 },
};
...
export default function App() {
 ...
 function yapAnaliz() {
 if (!txt.trim()) return;
 setSonuclar(analiz(txt, KATEGORILER[kategori].db));
 setAcik(null);
 ...
 <div style={{ color: C.altin, fontSize: 28, fontWeight: 700, letterSpacing: 1 }}>Besin Dedektifi</div>
 ...
 <a href={`mailto:besindedektifii@gmail.com?subject=Yeni Madde Bildirimi - ${KATEGORILER[kategori].ad}&body=Ürün adı: %0D%0AMadde: %0D%0AKategori: ${KATEGORILER[kategori].ad}`} target="_blank" rel="noopener noreferrer" style={{ display: "block", width: "100%", background: C.altin, color: C.bg, padding: "14px 18px", borderRadius: 12, border: "none", fontWeight: 700, fontSize: 14, marginBottom: 10, fontFamily: "inherit", cursor: "pointer", textDecoration: "none", textAlign: "center", boxSizing: "border-box" }}>Maddeyi Bildir</a>
 ...
 <div style={{ color: x.c, fontSize: 10, fontWeight: 700, letterSpacing: 1.5, marginTop: 4 }}>{x.l}</div>
 </div>
 ))}
 </div>
 );
 ...
 <div style={{ flex: 1 }}>
 <div style={{ color: C.altin, fontSize: 10, fontWeight: 700, letterSpacing: 1, marginBottom: 2 }}>{r.kod} · {r.kat}</div>
 ...
 <div style={{ color: C.cok, fontSize: 12, fontStyle: "italic" }}>{r.kaynak}</div>
 ...
 <div style={{ color: C.metin, fontSize: 13, fontStyle: "italic", lineHeight: 1.7 }}>"{ayet.metin}"</div>
 </div>
 )}
 ...
 <div style={{ color: C.cok, fontSize: 11, lineHeight: 1.6, fontStyle: "italic" }}>
 ...
 <div style={{ flex: 1 }}>
 <div style={{ color: C.altin, fontWeight: 700, fontSize: 16, letterSpacing: 0.5 }}>Besin Dedektifi</div>
 </div>
 ...
 {mod === "kamera" ? (
 <KameraOCR onMetin={m => { setTxt(m); setMod("metin"); }} onIptal={() => setMod("metin")} />
 ) : (
 <>
 ...
 <div style={{ color: C.cok, fontSize: 10, letterSpacing: 1, marginBottom: 4 }}>{k}</div>
 ...
 <div>
 <div style={{ color: C.altin, fontSize: 10, fontWeight: 700, letterSpacing: 1 }}>{kod} · {v.kat}</div>
 ...
 <span style={{ fontSize: 13, color: sekme === k ? C.altin : C.metin, fontWeight: sekme === k ? 700 : 500, letterSpacing: 0.3 }}>{label}</span>
 ...
 <div style={S.kB}>Kaynak</div><div style={{ color: C.cok, fontSize: 12, fontStyle: "italic", marginBottom: 12 }}>{modal.kaynak}</div>
